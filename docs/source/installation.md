...
6.  If using Postgres (e you can also browse `http://localhost:8001` for
    pgadmin. The default root password is [janeway-web]{.title-ref}
...
-   `make command` allows you to run django management commands using
    ...
    equivalent to running [python3 src/manage.py migrate
    journal]{.title-ref} in a native install)
...
-   `make shell`: Start a janeway container and run an interactive
    ...
    if a janeway server is running, in that case use [make
    attach]{.title-ref} instead)
...
-   
    ...
    If you want to run janeway against a different database vendor, you can use the [DB\_VENDOR]{.title-ref} variable. The following values are supported: `postgres` (default), `mysql` or `sqlite`

    :   -   e.g. if you want to install the development environment for
            Janeway using a mysql container, you can run
            `make install DB_VENDOR=mysql` for the installation and
            `make run DB_VENDOR=mysql` to spin up the mysql container
            alongside the Janeway development server.
...
-   By default, the database backend will come with a database named
    ...
    variable (e.g.: `make install DB_NAME="janeway_staging"` or
...
when you first run [make rebuild]{.title-ref} and you will be able to
make changes to the library without having to run `make rebuild`. In
order to install a library in development mode, copy the code to
`/path/to/janeway/lib/` and run `make rebuild` once.
...
2.  Optionally, copy `dockerfiles/lando_local.env.example` to
    ...
    attention to `dockerfiles/lando_local.env` if it\'s important to
    you)
3.  `lando poweroff` (defensively ensure no other Lando environments are
    running, probably not necessary, but a good habit)
4.  `lando rebuild`
5.  When you see the big \"Boomshakala\" message from Lando, you\'re
    ready to proceed
...
8.  `lando logs -f` will show you the log output from Janeway, though
    when you\'re getting started, [lando manage check]{.title-ref} will
    help you find configuration errors much faster than sifting through
    log file output
9.  `lando manage <command>` will send commands to the
    src/core/manage.py script, run [lando manage -h]{.title-ref} to see
    more info
...
13. Browse to [http://localhost:8000]{.title-ref} to see the site in
    action
...
::: {.toctree maxdepth="4" caption="SSO Providers"}
oidc orcid
:::
