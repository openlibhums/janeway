    ...
    def handle(self, *args, **options):
        ...
        if not options.get("path"):
            ...
            for file in get_modules():
                ...
                print(
                    "To run an upgrade use the following: `python3 manage.py run_upgrade --script 12_13`"
                )
        else:
            translation.activate("en")
            upgrade_module_name = options.get("path")
            upgrade_module_path = "utils.upgrade.{module_name}".format(
                module_name=upgrade_module_name
            )
