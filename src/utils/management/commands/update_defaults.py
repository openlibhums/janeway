...
def update_default_setting(default_data, db_setting):
    for default_setting in default_data:
        if (
            default_setting["setting"].get("name") == db_setting.setting.name
            and default_setting["group"].get("name") == db_setting.setting.group.name
        ):
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not group_name and not setting_name:
            setting_list = core_models.SettingValue.objects.filter(journal=journal)

        if group_name:
            setting_list = core_models.SettingValue.objects.filter(
                setting__group__name=group_name, journal=journal
            )
