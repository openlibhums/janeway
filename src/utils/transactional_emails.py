...
def send_submission_acknowledgement(**kwargs):
    ...
    editors_to_email = setting_handler.get_setting(
        "general", "editors_for_notification", request.journal
    ).processed_value
...
def send_revisions_complete(**kwargs):
    ...
    for action in revision.actions.all():
        action_text = "{0}<br><br>{1} - {2}".format(
            action_text, action.logged, action.text
        )

    description = "<p>{0} has completed revisions for {1}</p> Actions:<br>{2}".format(
        request.user.full_name(), article.title, action_text
    )
...
def send_copyedit_updated(**kwargs):
    ...
    if not skip:
        ...
        log_dict = {
            "level": "Info",
            "action_text": "Copyedit assignment #{number} update.".format(
        ...
        notify_helpers.send_email_with_body_from_setting_template(
            ...
            context={"request": request, "copyedit_assignment": copyedit_assignment},
            log_dict=log_dict,
        )
...
def send_copyedit_decision(**kwargs):
    ...
    log_dict = {
        "level": "Info",
        "action_text": description,
        "types": "Copyediting Decision",
        "target": copyedit_assignment.article,
    }

    notify_helpers.send_email_with_body_from_user(
        request,
        "subject_copyediting_decision",
...
def send_copyedit_complete(**kwargs):
    ...
    description = (
        "Copyediting requested by {0} from {1} for article {2} "
    ...
    article_copyediting_url = request.journal.site_url(
        reverse(
            "article_copyediting",
            args=[article.pk],
        )
...
def send_copyedit_ack(**kwargs):
    ...
    if not skip:
        log_dict = {
            "level": "Info",
            "action_text": description,
            "types": "Copyedit Acknowledgement",
            "target": copyedit_assignment.article,
        }
...
def send_typeset_assignment(**kwargs):
    ...
    description = "{0} has been assigned as a typesetter for {1}".format(
        typeset_task.typesetter.full_name(), typeset_task.assignment.article.title
    )
...
def send_typeset_decision(**kwargs):
    ...
    description = "{0} has {1}ed the typesetting task for {2}".format(
    ...
    log_dict = {
        "level": "Info",
        "action_text": description,
        "types": "Typesetter Decision",
        "target": typeset_task.assignment.article,
    }

    notify_helpers.send_email_with_body_from_user(
        request,
        "Article Typesetting Decision",
...
def send_typeset_task_deleted(**kwargs):
    ...
    description = (
        "{0} has deleted a typesetter task assigned to {1} for article {2}".format(
            request.user.full_name(),
            typeset_task.typesetter.full_name(),
            typeset_task.assignment.article.title,
        )
    ...
    notify_helpers.send_email_with_body_from_setting_template(
        ...
        typeset_task.typesetter.email,
        context={"request": request, "typeset_task": typeset_task},
        log_dict=log_dict,
    )
    notify_helpers.send_slack(request, description, ["slack_editors"])
...
def fire_proofing_manager_assignment(**kwargs):
    ...
    proofing_url = request.journal.site_url(
        reverse("proofing_article", args=[article.pk])
    )
...
def cancel_proofing_task(**kwargs):
    ...
    description = (
        "Proofing request for article {0} from {1} has been cancelled by {2}".format(
            article.title,
...
def edit_proofing_task(**kwargs):
    ...
    description = (
        "Proofing request for article {0} from {1} has been edited by {2}".format(
            article.title,
            proofing_task.proofreader.full_name(),
            request.user.full_name(),
        )
...
def notify_proofreader(**kwargs):
    ...
    description = (
        "Proofing request for article {0} from {1} has been requested by {2}".format(
            article.title,
            proofing_task.proofreader.full_name(),
            request.user.full_name(),
        )
    ...
    log_dict = {
        "level": "Info",
        "action_text": description,
        "types": "Proofreading Requested",
        "target": article,
    }
    notify_helpers.send_email_with_body_from_user(
        request,
        "subject_notify_proofreader_assignment",
        proofing_task.proofreader.email,
        user_content_message,
        log_dict=log_dict,
    )
...
def send_proofreader_decision(**kwargs):
    ...
    log_dict = {
        "level": "Info",
        "action_text": description,
        "types": "Proofreading Update",
        "target": proofing_task.round.assignment.article,
    }
    notify_helpers.send_email_with_body_from_user(
        request,
        "Article Proofreading Update",
...
def send_corrections_complete(**kwargs):
    ...
    description = (
        "{0} has completed corrections task for article {1} (proofing task {2})".format(
            request.user.full_name(),
            article.title,
            typeset_task.pk,
        )
...
def send_proofing_ack(**kwargs):
    ...
    description = "{0} has acknowledged a task , {1}, by {2} for article {3}".format(
...
def send_author_publication_notification(**kwargs):
    ...
    if peer_reviewers:
        reviewers = {
            review_assignment.reviewer
            for review_assignment in article.completed_reviews_with_decision
        }
...
def review_sec_override_notification(**kwargs):
    ...
    description = "{0} overrode their access to {1}".format(
...
def send_draft_decison(**kwargs):
    ...
    review_edit_draft_decision_url = request.journal.site_url(
        path=reverse("review_edit_draft_decision", args=[article.pk, draft.pk])
    )
    context = {
        "draft": draft,
        "article": article,
...
def send_author_copyedit_complete(**kwargs):
    ...
    description = (
        "Author {0} has completed their copyediting task for article {1}".format(
...
def access_request_notification(**kwargs):
    ...
    if request.journal:
        contact = request.journal.get_setting(
            "general", "submission_access_request_contact"
        )
    else:
    ...
    context = {
        ...
        "url": request.repository.site_url(
            path=reverse(
                ...
                kwargs={"preprint_id": review.preprint.pk, "review_id": review.pk},
            )
        ),
    }
    email_text = render_template.get_message_content(
