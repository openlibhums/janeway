...
def get_copyeditors(article):
    ...
    return core_models.AccountRole.objects.filter(
        role__slug="copyeditor", journal=article.journal
    ).exclude(
...
def get_copyeditor_notification_context(request, article, copyedit):
    ...
    """
    copyedit_requests_url = request.journal.site_url(reverse("copyedit_requests"))
    email_context = {
        "article": article,
        "assignment": copyedit,
...
def get_copyedit_message(request, article, copyedit, template, author_review=None):
    """
    Takes a set of variables and renders a template into a string.
    ...
    """
    if author_review:
        copyedit_review_url = request.journal.site_url(
            path=reverse("author_copyedit", args=[article.pk, author_review.pk])
        )
    else:
        copyedit_review_url = None

    copyedit_requests_url = request.journal.site_url(path=reverse("copyedit_requests"))
...
def handle_file_post(request, copyedit):
    ...
    if not errors:
        new_file = files.save_file_to_article(
            file, copyedit.article, request.user, label=file_label
        )
