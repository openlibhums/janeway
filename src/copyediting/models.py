...
class ActiveCopyeditAssignmentManager(models.Manager):
    def get_queryset(self):
        return (
            super(ActiveCopyeditAssignmentManager, self)
            .get_queryset()
            .exclude(
...
class CopyeditAssignment(models.Model):
    ...
    copyeditor = models.ForeignKey(
        "core.Account", related_name="copyeditor", null=True, on_delete=models.SET_NULL
    )
    editor = models.ForeignKey(
        "core.Account", related_name="cp_editor", null=True, on_delete=models.SET_NULL
    )
    ...
    editor_note = models.TextField(
        blank=True, null=True, verbose_name="Note for the Copyeditor"
    )
    copyeditor_note = models.TextField(
        blank=True, null=True, verbose_name="Note for the Editor"
    )

    decision = models.CharField(
        max_length=20, choices=copyeditor_decisions(), null=True, blank=True
    )
    date_decided = models.DateTimeField(null=True, blank=True)

    files_for_copyediting = models.ManyToManyField(
        "core.File", related_name="files_for_copyediting"
    )
    ...
    copyedit_reopened_complete = models.DateTimeField(
        blank=True, null=True, help_text="The date a reopen was complete"
    )
    ...
    def __str__(self):
        return "Assignment of {0} to {1}".format(
            self.copyeditor.full_name() if self.copyeditor else "",
            self.article,
        )
    ...
    def actions_available(self):
        if (
            ...
            and self.copyeditor_completed
            and not self.copyedit_reopened
            and not self.copyedit_accepted
            and not self.incomplete_author_reviews()
        ):
            return True
        elif (
            self.decision == "accept"
            and self.copyeditor_completed
            and self.copyedit_reopened_complete
            and not self.copyedit_accepted
            and not self.incomplete_author_reviews()
        ):
            return True
    ...
    def copyedit_log(self):
        ...
        log.append(
            {
                "date": self.assigned,
                "event": "Copyedit request assigned",
                "slug": "assigned",
            }
        )
        for review in self.author_reviews():
            ...
            if review.date_decided:
                log.append(
                    {
                        "date": review.date_decided,
                        "event": "Author completed review",
        ...
        if self.copyeditor_completed:
            log.append(
                {
                    "date": self.copyeditor_completed,
                    "event": "Initial copyedit complete",
                    "slug": "initial",
                }
            )
        if self.copyedit_reopened:
            log.append(
                {
                    "date": self.copyedit_reopened,
                    "event": "Copyedit reopened",
                    "slug": "reopened",
                }
            )
            ...
            log.append(
                {
                    ...
                    "event": "Copyeditor request decision: {0}".format(self.decision),
                    "slug": "decision",
                }
            )
        if self.copyedit_reopened_complete:
            log.append(
                {
                    "date": self.copyedit_reopened_complete,
                    "event": "Copyeditor completed additional edits",
        ...
        if self.copyedit_accepted:
            log.append(
                {
                    "date": self.copyedit_accepted,
                    "event": "Editor accepted the copyedit",
                    "slug": "accepted",
                }
            )
...
class ActiveAuthorReviewManager(models.Manager):
    def get_queryset(self):
        return (
            super(ActiveAuthorReviewManager, self)
            .get_queryset()
            .exclude(
...
class AuthorReview(models.Model):
    ...
    author_note = models.TextField(
        null=True, blank=True, verbose_name="Note to the Editor"
    )
