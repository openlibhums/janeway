    ...
    for journal in journal_models.Journal.objects.all():
        if journal.use_crossref:
            ...
            file_path = os.path.join(
                ...
                "files",
                "temp",
                "{prefix}.json".format(prefix=crossref_prefix),
            )
            with open(file_path, encoding="utf-8") as json_file:
                ...
                for event in json_data["message"]["events"]:
                    ...
                    try:
                        identifier = ident_models.Identifier.objects.get(
...
def fetch_crossref_data():
    ...
    for prefix in journal_prefixes:
        ...
        if not os.path.isfile(file_path):
            r = requests.get(
                "https://query.eventdata.crossref.org/events?rows=10000&filter=obj-id.prefix:{0}".format(
                    prefix
                )
            )
