...
class TestLogic(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.article_published = helpers.create_article(
            cls.journal_one, with_author=True
        )
    ...
    def test_create_crossref_doi_batch_context(self):
        ...
        context = logic.create_crossref_doi_batch_context(
            self.journal_one, set([self.doi_one])
        )
    ...
    def test_create_crossef_issues_context(self):
        ...
        identifiers.add(
            self.doi_four
        )  # Should be on its own due to Article.ISSN_override
        identifiers.add(
            self.doi_five
        )  # Should be on its own due to Article.publication_title
    ...
    def test_preview_registration_information_when_use_crossref_on(self):
        ...
        self.assertTrue(
            "Current metadata to send to Crossref"
            in self.article_one.registration_preview
        )
    ...
    def test_preview_registration_information_when_custom_doi(self):
        clear_cache()
        self.assertTrue(
            self.doi_three.identifier in self.article_three.registration_preview
        )
    ...
    def test_deposit_xml_document_is_valid(self):
        ...
        for journal in [self.journal_one, self.journal_two]:
            ...
            identifiers = set(
                [
                    identifier
                    for identifier in models.Identifier.objects.filter(
