...
def handle_closing_active_task(request, article):
    if article.proofingassignment.current_proofing_round().has_active_tasks:
        for (
            task
        ) in article.proofingassignment.current_proofing_round().proofingtask_set.all():
            if not task.completed:
                ...
                kwargs = {
                    ...
                    "proofing_task": task,
                    "request": request,
                    "user_content_message": request.POST.get(
                        "note_to_proofreaders", None
                    ),
                }
...
def get_all_possible_proofers(journal, article):
    active_proofreaders = (
        article.proofingassignment.current_proofing_round().active_proofreaders
    )
    ...
    proofers = (
        ...
        .exclude(user__in=article.authors.all())
        .exclude(user__in=active_proofreaders)
    )
    all_possible_proofers = [
        {"user": role.user, "role": role.role.slug} for role in proofers
    ]
...
def get_user_from_post(request, article=False, typesetter=False):
    ...
    if user_id:
        ...
        # return None if we aren't checking for typesetting and the user isn't a proofreder
            ...
            user.is_proofreader(request) or user.is_editor(request)
        ):
            return None
...
def get_galleys_from_post(request):
    ...
    return [
        core_models.Galley.objects.get(pk=galley_id) for galley_id in galley_id_list
    ]
...
def create_html_snippet(note, proofing_task, galley):
    ...
    html_content = template.render(
        {"note": note, "proofing_task": proofing_task, "galley": galley}
    )
...
def handle_proof_decision(request, proofing_task_id, decision):
    proofing_task = get_object_or_404(
        models.ProofingTask, proofreader=request.user, pk=proofing_task_id
    )
    if decision == "accept":
    ...
    messages.add_message(
        request,
        messages.INFO,
        "Proofing Task {0} decision: {1}".format(proofing_task_id, decision),
    )
...
def handle_typeset_decision(request, typeset_task_id, decision):
    ...
    messages.add_message(
        request,
        messages.INFO,
        "Typeset Task {0} decision: {1}".format(typeset_task_id, decision),
    )
...
def get_ack_message(request, article, model_name, model_object):
    ...
    return render_template.get_message_content(
        request, context, "thank_proofreaders_and_typesetters"
    )
...
def get_complete_proofing_message(request, article):
    ...
    return render_template.get_message_content(
        request, context, "notify_editor_proofing_complete"
    )
