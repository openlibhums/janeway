...
class ArticleSearchManager(BaseSearchManagerMixin):
    ...
    def postgres_search(self, search_term, search_filters, sort=None, site=None):
        ...
        if "relevance" in sort:
            ...
            return Article.objects.raw(
                f"SELECT * from ({inner_sql}) AS search " f"{order_by_sql}"
            )
...
class Article(AbstractLastModifiedModel):
    ...
    def how_to_cite(self):
        ...
        if doi:
            doi_str = (
                'doi: <a href="https://doi.org/{0}">' "https://doi.org/{0}</a>".format(
                    doi
                )
            )
...
class FrozenAuthor(AbstractLastModifiedModel):
    ...
    frozen_biography = JanewayBleachField(
        ...
        help_text=_(
            "The author's biography at the time they published"
    ...
    frozen_orcid = models.CharField(
        ...
        help_text=_(
            "ORCiD to be displayed when no account is"
    ...
    display_email = models.BooleanField(
        default=False,
        help_text=_(
            "If checked, this authors email address link will be displayed on the article page."
        ),
    )
    ...
    def is_correspondence_author(self):
        ...
        if not self.author or settings.DUMMY_EMAIL_DOMAIN in self.author.email:
            return False
