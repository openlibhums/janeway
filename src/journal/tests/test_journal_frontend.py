...
class TestJournalSite(TestCase):
    def setUp(self):
        ...
        self.new_user = helpers.create_user(
            "new_user@janeway.systems", ["author"], self.journal, **{"is_active": True}
        )
        ...
        self.collection, c = models.Issue.objects.get_or_create(
            ...
            defaults={"date": timezone.now(), "issue_title": "Test Collection"},
        )
    ...
    def test_search_includes_article(self):
        ...
        response = self.client.get(url, SERVER_NAME=self.journal_domain)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response,
            self.article_title,
