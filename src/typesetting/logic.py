...
def get_typesetting_files(article, previous_round=None):
    ...
    queryset = queryset.annotate(
        checked=ExpressionWrapper(checked_query, output_field=BooleanField())
    )
...
def get_proofreaders(article, round, assignment=None):
    pks = list()
    current_proofer_pks = [p.proofreader.pk for p in round.galleyproofing_set.all()]

    for editor in article.editor_list():
        pks.append(editor.pk)

    for author in article.author_accounts:
        pks.append(author.pk)

    for proofreader in core_models.AccountRole.objects.filter(
        role__slug="proofreader", journal=article.journal
    ):
...
OPEN_TASKS = _("One or more typesetting or proofing tasks haven't been completed")
