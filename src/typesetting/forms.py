...
class TypesettingAssignmentForm(forms.ModelForm, core_forms.ConfirmableIfErrorsForm):
    ...
    def check_for_potential_errors(self):
        ...
        if not files_to_typeset:
            message = (
                "No files were selected for typesetting. The typesetter "
        ...
        for galley_id in galleys_to_correct:
            try:
                ...
                if galley.file not in files_to_typeset:
                    message = (
                        f"The galley with a label of {galley.label} "
...
class ProofingForm(forms.ModelForm, core_forms.ConfirmableForm):
    ...
    def check_for_potential_errors(self):
        ...
        if annotated_files:
            ...
            last_editor_or_typesetter_action = max(
                filter(
                    bool,
                    [
