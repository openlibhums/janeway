...
class DraftDecisionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["revision_request_due_date"].widget.attrs["onchange"] = (
            "decision_change()"
        )
...
class ReviewAssignmentForm(forms.ModelForm, core_forms.ConfirmableIfErrorsForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.journal:
            self.fields["form"].queryset = models.ReviewForm.objects.filter(
                journal=self.journal, deleted=False
            )
        ...
        if default_form and not self.instance.form:
            try:
                self.fields["form"].initial = models.ReviewForm.objects.get(
                    pk=default_form
                )
    ...
    def check_for_potential_errors(self):
        ...
        one_click_access = setting_handler.get_setting(
            "general", "enable_one_click_access", self.journal
        ).value
...
class ReviewerDecisionForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["permission_to_make_public"].widget.attrs["checked"] = (
            open_review_initial
        )
...
class GeneratedForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        for element in elements:
            if element.kind == "text":
                ...
                self.fields[str(element.pk)] = forms.CharField(
                    widget=forms.DateInput(
                        attrs={"class": "datepicker", "div_class": element.width}
                    ),
                    required=element.required if fields_required else False,
                )
            ...
            if answer:
                self.fields[str(element.pk)].initial = (
                    answer.edited_answer if answer.edited_answer else answer.answer
                )
            ...
            if review_assignment:
                ...
                if answers:
                    try:
                        ...
                        models.ReviewFormAnswer.DoesNotExist,
                        models.ReviewAssignmentAnswer.DoesNotExist,
                    ):
                        pass
