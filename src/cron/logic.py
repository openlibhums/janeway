...
def process_editor_digest(journal, user_role):
    unassigned_articles = submission_models.Article.objects.filter(
        stage=submission_models.STAGE_UNASSIGNED, journal=journal
    )
    overdue_reviews = review_models.ReviewAssignment.objects.filter(
    ...
    overdue_revisions = review_models.RevisionRequest.objects.filter(
        date_completed__isnull=True,
        date_due__lte=timezone.now(),
        article__journal=journal,
    )
    overdue_proofing = proofing_models.ProofingTask.objects.filter(
        completed__isnull=True,
        cancelled__isnull=True,
        due__lte=timezone.now(),
        round__assignment__article__journal=journal,
    )
    awaiting_publication = submission_models.Article.objects.filter(
        stage=submission_models.STAGE_READY_FOR_PUBLICATION, journal=journal
    )
...
def process_revision_digest(journal, user_role):
    pending_requests = review_models.RevisionRequest.objects.filter(
        article__correspondence_author=user_role.user,
        date_completed__isnull=True,
        date_due__gte=timezone.now(),
    )
    overdue_requests = review_models.RevisionRequest.objects.filter(
        article__correspondence_author=user_role.user,
        date_completed__isnull=True,
        date_due__lte=timezone.now(),
    )
