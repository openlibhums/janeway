...
class SettingAdmin(admin.ModelAdmin):
    ...
    inlines = [admin_utils.SettingValueInline]
...
class FileAdmin(admin.ModelAdmin):
    ...
    list_filter = (
        admin_utils.ArticleIDJournalFilter,
        "last_modified",
        "date_uploaded",
        "mime_type",
        "is_galley",
    ...
    def article_pk(self, obj):
        if obj and obj.article_id:
            link = '<a href="/admin/submission/article/{pk}/change/">{pk}</a>'.format(
                pk=obj.article_id
            )
    ...
    def _journal(self, obj):
        if obj and obj.article_id:
            return journal_models.Journal.objects.get(article__pk=obj.article_id).code
...
class SupplementaryFileAdmin(admin.ModelAdmin):
    ...
    list_filter = (
        admin_utils.FileArticleIDJournalFilter,
        "file__date_uploaded",
        "file__last_modified",
        "file__mime_type",
    )
...
class GalleyAdmin(admin.ModelAdmin):
    ...
    def file_link(self, obj):
        if obj and obj.file:
            link = '<a href="/admin/core/file/{pk}/change/">{pk}</a>'.format(
                pk=obj.file.pk
            )
            return mark_safe(link)
        else:
        ...
        if search_term:
            queryset = queryset.annotate(
                relevance=Case(
                    ...
                    When(custom_label__value__istartswith=search_term, then=Value(70)),
                    When(ror_display__value__istartswith=search_term, then=Value(70)),

                    When(ror_id__icontains=search_term, then=Value(60)),
                    When(custom_label__value__icontains=search_term, then=Value(50)),
...
class LocationAdmin(admin.ModelAdmin):
    ...
    list_filter = ("country",)
    search_fields = ("pk", "name", "country__code", "country__name", "geonames_id")
