...
class CoreTests(TestCase):
    ...
    def test_mixed_case_login_same_case(self):
        ...
        self.assertEqual(
            ...
            str(account.pk),
            msg="Registered user %s can't login with email %s" % (email, email),
        )

    orcid_record = {
        "orcid": "0000-0000-0000-0000",
        "uri": "http://sandbox.orcid.org/0000-0000-0000-0000",
        "emails": ["campbell@evu.edu"],
        "last_name": "Kasey",
        "first_name": "Campbell",
        "affiliations": [
            {
                "organization": {
                    "name": "Elk Valley University",
                    "address": {"country": "US"},
                }
            }
        ],
    ...
    def test_register_with_orcid_token(self, record_mock):
        ...
        response = self.client.post(
            register_url,
            {
                "first_name": "Campbell",
                "last_name": "Kasey",
                "email": "campbell@evu.edu",
                "institution": "Elk Valley University",
        ...
        self.assertTrue(
            models.Account.objects.filter(email="campbell@evu.edu").exists()
        )
    ...
    def test_mixed_case_login_different_case(self):
        ...
        self.assertEqual(
            self.client.session["_auth_user_id"],
            str(account.pk),
            msg="Registered user %s can't login with email %s" % (email, login_email),
        )
    ...
    def test_register_as_reader(self):
        ...
        clear_cache()
        response = self.client.post(
            reverse("core_register"), data, SERVER_NAME="testserver"
        )
    ...
    def test_email_subjects(self):
        ...
        missing = set(email_settings) - {s[len("subject_") :] for s in subject_settings}
        self.assertFalse(missing, msg="Found emails that don't have a subject setting")
    ...
    def test_hide_review_details_on(self):
        ...
        ).update(for_author_consumption=False)
    ...
    def test_peer_reviews_for_author_consumption_overrides_hide_review_data(self):
        ...
        review_models.ReviewAssignment.objects.filter(
            article=self.article_one,
        ).update(for_author_consumption=True)
