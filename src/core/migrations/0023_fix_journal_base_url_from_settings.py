...
def replace_journal_base_url(apps, schema_editor):
    ...
    for setting in settings:
        setting.value = setting.value.replace(
            "request.journal_base_url", "journal.site_url"
        )
        setting.save()
...
def reverse_code(apps, schema_editor):
    ...
    for setting in settings:
        setting.value = setting.value.replace(
            "journal.site_url", "request.replace_journal_base_url"
        )
        setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            replace_journal_base_url, reverse_code=migrations.RunPython.noop
        ),
    ]
